...
//         "goalName": goal.name,
//         "startDate":
//             goal.startDate.toIso8601String().split('T')[0], // "YYYY-MM-DD" 형식
...
//     print(
//         'updateProgress() called with goalId: $goalId and progress: $progress');
//     final token = await authService.getToken();
//     if (token == null) {
//       throw Exception('JWT 토큰이 없습니다. 다시 로그인해주세요.');
//     }
...
//               DateTime.now().isAfter(updatedGoal.endDate)) {
//             if (updatedGoal.status != GoalStatus.completed) {
//               updatedGoal.status = GoalStatus.completed;
//               print(
//                   'Goal automatically marked as completed: ${updatedGoal.name}');
...
//     final List<Goal> unsyncedGoals =
//         goalBox.values.where((g) => !g.isSynced).toList();
