  ...
  group('TodoManageViewModel', () {
    ...
    group('날짜 및 필터 업데이트', () {
      ...
      test('목표별 필터링 시 해당 목표의 투두만 표시되어야 한다', () {
        ...
        expect(viewModel.dDayTodos.every((todo) => todo.goalId == targetGoalId), isTrue);
        expect(viewModel.dailyTodos.every((todo) => todo.goalId == targetGoalId), isTrue);
      });
      ...
      test('중요도별 필터링 시 중요도가 높은 투두만 표시되어야 한다', () {
        ...
        expect(viewModel.dDayTodos.length, 1);
        expect(viewModel.dailyTodos.length, 1);
        expect(viewModel.dDayTodos.every((todo) => todo.importance == 1), isTrue);
        expect(viewModel.dailyTodos.every((todo) => todo.importance == 1), isTrue);
      });
      ...
      test('목표 선택 시 해당 목표의 투두만 필터링되어야 한다', () {
        ...
        expect(viewModel.dDayTodos.length, 1);
        expect(viewModel.dailyTodos.length, 1);
        expect(viewModel.dDayTodos.every((todo) => todo.goalId == 'goal-2'), isTrue);
        expect(viewModel.dailyTodos.every((todo) => todo.goalId == 'goal-2'), isTrue);
      });
    });
    ...
    group('투두 관리', () {
      ...
      test('updateTodoStatus는 투두 상태를 업데이트해야 한다', () async {
        ...
        when(mockUpdateTodoStatusUseCase.call(targetTodo, newStatus))
        ...
        // Then
        verify(mockUpdateTodoStatusUseCase.call(targetTodo, newStatus)).called(1);
        
        // 업데이트된 투두를 확인 - 내부 리스트에서 상태가 변경되어야 함
        final updatedTodo = viewModel.allTodos.firstWhere((todo) => todo.id == targetTodo.id);
      ...
      test('deleteTodoById는 투두를 삭제하고 데이터를 다시 로드해야 한다', () async {
        ...
        when(mockDeleteTodoUseCase.call(targetTodo))
            .thenAnswer((_) => Future<void>.value());
      ...
      test('updateTodoDates는 투두 날짜를 업데이트하고 데이터를 다시 로드해야 한다', () async {
        ...
        when(mockUpdateTodoDatesUseCase.call(targetTodo, newStartDate, newEndDate))
            .thenAnswer((_) => Future<void>.value());
        ...
        // Then
        verify(mockUpdateTodoDatesUseCase.call(targetTodo, newStartDate, newEndDate)).called(1);
